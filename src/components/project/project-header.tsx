...
export function ProjectHeader({ project }: ProjectHeaderProps) {
  ...
  React.useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 10); // Reduced threshold for earlier compacting
    };
  ...
  return (
    <div className={`space-y-1 transition-all duration-200 ${isScrolled ? 'py-0' : 'pb-1'}`}>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-1">
        <h1 className={`font-bold tracking-tight transition-all duration-200 ${isScrolled ? 'text-base' : 'text-xl'}`}>
          {project.title}
        </h1>
        <div className="flex flex-wrap gap-2">
          {/* Submission date badge */}
          {formattedSubmissionDate && (
            <Badge variant="outline" className="flex items-center whitespace-nowrap text-xs">
      ...
      {!isScrolled && project.description && project.description.document && project.description.document[0]?.children && (
        <div className="text-muted-foreground text-sm">
          <div className="prose-xs dark:prose-invert max-w-none line-clamp-2">
            <DocumentRenderer
