...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project }) => {
  ...
  const formatStudentInfo = () => {
        ...
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          <div className="flex items-center gap-1.5">
            {project.students.length > 1 ? (
              <>
                <UsersIcon size={14} />
                <span>Samenwerking: {project.students.map(s => s.name).join(', ')}</span>
              </>
            ) : (
              <>
                <UserIcon size={14} />
                <span>{project.students[0].name}</span>
              </>
            )}
          </div>
          {project.students[0].class && (
            <div className="flex items-center gap-1.5">
              <span>•</span>
              <span>{project.students[0].class.name}</span>
            </div>
          )}
          {project.students[0].graduationYear && (
            <div className="flex items-center gap-1.5">
              <span>•</span>
              <GraduationCapIcon size={14} />
              <span>{project.students[0].graduationYear}</span>
            </div>
          )}
          <div className="flex items-center gap-1.5">
            <span>•</span>
            <TagIcon size={14} />
            <span>{projectTypeLabels[project.projectType || 'other'] || project.projectType}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span>•</span>
    ...
    // Fall back to legacy student property
    if (project.student) {
      return (
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          <div className="flex items-center gap-1.5">
    ...
    // If no student info is available
    return (
      <div className="flex items-center gap-3 text-sm text-muted-foreground">
        <div className="flex items-center gap-1.5">
          <TagIcon size={14} />
  ...
  // Function to render the reflection section with student avatars
  const renderReflectionSection = () => {
    // Multiple students
    if (project.students && project.students.length > 0) {
      if (project.students.length === 1) {
        // Single student from students array
        return (
          <div className="flex items-center gap-4 mb-4">
            <InitialsAvatar name={project.students[0].name} size="lg" />
            <div>
              <h3 className="font-semibold text-lg">{project.students[0].name}'s Reflection</h3>
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="flex -space-x-2">
              ...
              {project.students.length > 3 && (
                <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-sm border-2 border-background">
                  +{project.students.length - 3}
                </div>
              )}
            </div>
            <div>
              <h3 className="font-semibold text-lg">Team Reflection</h3>
              <p className="text-sm text-muted-foreground">Collaborative development process</p>
            </div>
          </div>
        );
      }
    ...
    // Fallback to legacy student property
    if (project.student) {
      return (
        <div className="flex items-center gap-4 mb-4">
    ...
    // Default if no student info
    return (
      <div className="mb-4">
        <h3 className="font-semibold text-lg">Project Reflection</h3>
        <p className="text-sm text-muted-foreground">Development process and challenges</p>
      </div>
    );
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        {/* Student/creator info - updated to handle multiple students */}
        {formatStudentInfo()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* Code Files */}
          {project.files && project.files.length > 0 && project.files.some(cf => cf.file) && (
            <div className="bg-card rounded-lg border p-4 space-y-3">
              <div className="flex items-center gap-2">
              ...
              <div className="space-y-3">
                {project.files.map((codeFile, index) => (
                  codeFile.file && (
